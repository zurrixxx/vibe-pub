...
export const GET: RequestHandler = async ({ url, platform }) => {
  ...
  if (!tokenData.access_token) {
    throw error(400, `Google auth failed: ${tokenData.error ?? 'unknown error'}`);
  }
  ...
  const profile = await userRes.json() as { email: string; verified_email: boolean; name?: string };
  ...
  if (!user) {
    ...
    const rawUsername = profile.email.split('@')[0].replace(/[^a-z0-9_]/gi, '_').toLowerCase();
