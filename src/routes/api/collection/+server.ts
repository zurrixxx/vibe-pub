...
export const POST: RequestHandler = async ({ request, locals, platform }) => {
  ...
  await db
    ...
    .bind(id, collectionSlug, title, description ?? null, locals.user?.id ?? null, access ?? 'unlisted', theme ?? 'default')
    .run();
  ...
  for (let i = 0; i < page_slugs.length; i++) {
    ...
    if (pageId) {
      await db
        .prepare('INSERT INTO collection_pages (collection_id, page_id, sort_order) VALUES (?, ?, ?)')
        .bind(id, pageId, i)
        .run();
    }
  }
