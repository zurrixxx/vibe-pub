...
export async function getPageBySlug(db: D1Database, slug: string, userId?: string): Promise<Page | null> {
  if (userId) {
    return db.prepare('SELECT * FROM pages WHERE slug = ? AND user_id = ?').bind(slug, userId).first<Page>();
  }
  return db.prepare('SELECT * FROM pages WHERE slug = ? AND user_id IS NULL').bind(slug).first<Page>();
}

export async function getPagesByUser(db: D1Database, userId: string): Promise<Page[]> {
  const result = await db.prepare('SELECT * FROM pages WHERE user_id = ? ORDER BY updated DESC').bind(userId).all<Page>();
